...
If two values have equal frequency and equal value → they are identical, so no conflict.
...
Brute force:
- For each element → count by scanning entire array.
- Time = O(N²).
...
1. Traverse array once → build frequency map.
...
Invariant:
Frequency constant → ordering depends entirely on value.
...
Between 2 and 3 → smaller value first.
...
### What if input size increases to 10⁷?
